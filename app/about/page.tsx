...
export default function AboutPage(){
  return <main>
    ...
    <style>{`
      .about-story-hero{position:relative;min-height:760px;display:flex;align-items:center;overflow:hidden;background:url('/brand/jj-sidewalk.webp') center 42%/cover no-repeat;border-bottom:1px solid var(--line)}
      .about-story-shade{position:absolute;inset:0;background:linear-gradient(90deg,rgba(3,10,6,.92) 0%,rgba(3,10,6,.78) 37%,rgba(3,10,6,.42) 64%,rgba(3,10,6,.18) 100%),linear-gradient(0deg,rgba(4,12,7,.38),rgba(4,12,7,.08))}
      ...
      @media(max-width:800px){.about-story-hero{min-height:720px;background-position:63% center}.about-story-shade{background:linear-gradient(90deg,rgba(3,10,6,.94),rgba(3,10,6,.7) 65%,rgba(3,10,6,.42))}.about-story-copy{margin-left:24px;padding:90px 0;width:calc(100% - 48px)}.about-story-copy h1{font-size:64px}}
    `}</style>
  </main>
}
